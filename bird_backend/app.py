...
yolo_model.eval()  # Set YOLO model to evaluation mode, ensuring no dropout etc

BIRD_CLASS = 'bird'  # Update according to YOLO's class names (bird is class 14)

@app.route('/', methods=['GET'])
def home():
    return jsonify({"message": "Welcome to SmartBirds.io API"}), 200

 # Function to detect and crop bird using YOLO
def detect_and_crop(image: Image.Image) -> (Image.Image, list):
    # Convert PIL Image to OpenCV image
    cv_image = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR)
    results = yolo_model(cv_image)

    # Filter detections for birds
    birds = [det for det in results.xyxy[0] if results.names[int(det[5])] == BIRD_CLASS]

    if not birds:
        raise ValueError("No birds detected in the image.")

    # Assuming the first detection is the bird of interest
    bird = birds[0]
    x1, y1, x2, y2, conf, cls = bird
    cropped_cv_image = cv_image[int(y1):int(y2), int(x1):int(x2)]
...
# Configuration
MODEL_PATH = 'bird_vit_model.pth'  # Local path to the model
...
def predict():
    ...
    try:
        # Detect and crop bird from the image
        cropped_image, bbox = detect_and_crop(image)
    except Exception as e:
        return jsonify({'error': f'Error during detection: {e}'}), 400
    
    # Transform the cropped image
    img_t = transform(cropped_image).unsqueeze(0).to(device)
    ...
    return jsonify({
        ...
        'bounding_box': bbox  # Return bounding box
    }), 200
